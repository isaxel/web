...
**HTTP** — протокол прикладного уровня, основанный на модели «клиент-сервер». Используется для передачи гипертекста (обычно HTML) между клиентом (браузером) и сервером.
...
- **URL (Uniform Resource Locator)** — URI, который указывает местоположение ресурса (схема + путь). Пример: `http://example.com/page.html`.
- **URN (Uniform Resource Name)** — URI, который именует ресурс, но не указывает его местоположение. Пример: `urn:isbn:0451450523`.

**Примеры:**
- URL: `http://cs.ifmo.ru/spip.html`
- URN: `urn:sha1:YNCKHTQCMBTRNJIV4WNAE52SJUQCZ05C`
